        ...
        <overlay-scrollbars class="nav-scrollbar">
            <v-list class="pr-0 pt-0 ml-0">
                <v-list-item-group active-class="active-nav-item">
                    <v-list-item v-if="isMobile" router to="/" :class="
                    ...
                    " :style="'height: ' + topbarHeight + 'px'" :ripple="false">
                        ...
                        <template v-else>
                            <mainsail-logo :color="logoColor" :style="logoCssVars" class="nav-logo"
                    ...
                    <template v-if="countPrinters">
                        <v-tooltip right :open-delay="500" :disabled="navigationStyle !== 'iconsOnly'">
                            <template #activator="{ on, attrs }">
                                <v-list-item router to="/allPrinters" class="small-list-item mt-1" v-bind="attrs" v-on="on">
                    ...
                    <div v-for="(category, index) in naviPoints" :key="index">
                        <v-tooltip right :open-delay="500" :disabled="navigationStyle !== 'iconsOnly'">
                            <template #activator="{ on, attrs }">
                                <v-list-item router :to="category.path" class="small-list-item" v-bind="attrs" v-on="on">
                                    <v-list-item-icon class="my-3 mr-3 menu-item-icon">
...
export default class TheSidebar extends Mixins(BaseMixin) {
    ...
    // The access tabs that are shown on the sidebar
    get naviPoints(): AppRoute[] {

        return routes.filter((element) => {

            if (adminState.isAdmin == false) {
                return element.showInNavi && this.showInNavi(element) && element.admin == false
            } else {
